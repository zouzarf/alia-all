...
function Event({ event }: { event: events_logs }) {
    ...
    return (<div className="relative w-full">
        ...
        <div className="ml-6">
            <h4 className="font-bold text-blue-500">Exection of : {event.job_full_date?.toUTCString()}</h4>
            <p className="mt-2 max-w-screen-sm text-sm text-gray-500">Start time: {event.process_start?.toUTCString()}</p>
            <p className="mt-2 max-w-screen-sm text-sm text-gray-500">End time: {event.process_end?.toUTCString()}</p>
            <span className="mt-1 block text-sm font-semibold text-blue-500">Time: {formatTime((event.process_end!.getTime() - event.process_start!.getTime()) / 1000)}s</span>
        </div>
    </div>)
